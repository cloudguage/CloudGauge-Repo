...
def Orchestrator(subscription_name):
    ...
    try:
        ...
        master_configuration_list=list_files_by_name(credentials,"CloudGauge-Master-Configuration")
                    ...
                    if params['IsScope']=="Yes":
                        ...
                        if params['ConfigType']=="Azure-SQLDB-Query":
                            if not azure_sqldb_query(params):
                                flag=0
                        #GCP
                        if params['ConfigType']=="GCP-BigQuery-Query" :
                            if not gcp_bigquery_query(params):
                                flag=0
                        if params['ConfigType']=="GCP-FireStore-Query" :
                            if not gcp_firestore_query(params):
                                flag=0
                        if params['ConfigType']=="GCP-Function-Execute" :
                            if not gcp_function_execute(params):
                                flag=0
                        if params['ConfigType']=="GCP-WebApp-Connect" :
                            if not gcp_webapp_connect(params):
                                flag=0
                        #AWS     
                        if params['ConfigType']=="AWS-RDSPostgres-Query" :
                            if not aws_rds_query(params):
                                flag=0
                        if params['ConfigType']=="AWS-DynamoDB-Query" :
                            if not aws_dynamodb_query(params):
                                flag=0
                        if params['ConfigType']=="AWS-WebApp-Connect" :
                            if not aws_webapp_connect(params):
                                flag=0
                        #IBM   
                        if params['ConfigType']=="IBM-DB2-Query" :
                            if not ibm_db2_query(params):
                                flag=0
                        if params['ConfigType']=="IBM-Cloudant-Query" :
                            if not ibm_cloudant_query(params):
                                flag=0
                        if params['ConfigType']=="IBM-WebApp-Connect" :
